...
**Architecture:** Layered pipeline with a language-agnostic core (`tokenize`, `score`, `align`) and pluggable language packs (`lang.<code>`). All public types live in `src/trace/model.py`. The Hebrew pack auto-registers on import. See `docs/superpowers/specs/2026-04-28-trace-v0.1-design.md` for full design.
...
| 16 | Public API in `src/trace/__init__.py` |
...
The distribution name is `trace-align` (the `trace` PyPI name is taken and conflicts with the stdlib `trace` module). The Python import path stays `import trace` because the user's existing `src/trace/` directory is the source of truth; the stdlib shadow is accepted for v0.1 and revisited if it causes problems.
...
Expected: install succeeds, `trace-align` is registered.
...
```bash
git add pyproject.toml .flake8 requirements-dev.txt src/trace/__init__.py tests/__init__.py tests/conftest.py
...
Expected: `ImportError: cannot import name 'Match' from 'trace.model'` (module does not exist yet).
...
Run: `flake8 src/trace/model.py tests/test_model.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/model.py tests/test_model.py
...
from trace.model import AlignmentResult, Lexica
...
**Files:**
- Create: `src/trace/tokenize/__init__.py`
- Create: `src/trace/tokenize/base.py`
...
from trace.tokenize.base import (
...
Expected: ImportError, no module `trace.tokenize.base`.

- [ ] **Step 3: Implement base types**

```python
# src/trace/tokenize/__init__.py
"""Tokenizer subpackage."""
```

```python
# src/trace/tokenize/base.py
...
Run: `flake8 src/trace/tokenize tests/test_tokenize_base.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/tokenize tests/test_tokenize_base.py
...
**Files:**
- Create: `src/trace/tokenize/plaintext.py`
...
from trace.tokenize.base import DEFAULT_EDITORIAL_RULES, RawToken
from trace.tokenize.plaintext import pretokenize
...
```python
# src/trace/tokenize/plaintext.py
...
Run: `flake8 src/trace/tokenize tests/test_tokenize_plaintext.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/tokenize/plaintext.py tests/test_tokenize_plaintext.py
...
**Files:**
- Create: `src/trace/lang/__init__.py`
- Create: `src/trace/lang/base.py`
- Create: `src/trace/lang/registry.py`
...
from trace.lang.base import LanguagePack, ScoringTier, TierResult
from trace.lang.registry import (
...
from trace.model import Reason, Token
from trace.tokenize.base import RawToken
...
Expected: ImportError on `trace.lang`.

- [ ] **Step 3: Implement `lang.base` and `lang.registry`**

```python
# src/trace/lang/__init__.py
"""Language packs and registry."""
```

```python
# src/trace/lang/base.py
...
from trace.model import Lexica, Reason, Token
from trace.tokenize.base import (
    DEFAULT_EDITORIAL_RULES,
    EditorialBracketRules,
    RawToken,
...
```python
# src/trace/lang/registry.py
"""Registry for language packs."""

from __future__ import annotations

from trace.lang.base import LanguagePack
...
Run: `flake8 src/trace/lang tests/lang/test_registry.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/lang tests/lang/test_registry.py tests/lang/__init__.py
...
**Files:**
- Create: `src/trace/lang/hebrew/__init__.py`
- Create: `src/trace/lang/hebrew/pack.py`
- Create: `src/trace/lang/hebrew/tokenize.py`
...
from trace.lang.hebrew.tokenize import split_maqqef_compounds
from trace.tokenize.base import RawToken
from trace.tokenize.plaintext import pretokenize
...
```python
# src/trace/lang/hebrew/__init__.py
"""Hebrew language pack."""
```

```python
# src/trace/lang/hebrew/tokenize.py
...
```python
# src/trace/lang/hebrew/pack.py
"""HebrewLanguagePack — wires hooks together. Normalize and scoring come later."""

from __future__ import annotations

from trace.lang.base import LanguagePack, ScoringTier
from trace.lang.hebrew.tokenize import HEB_MID_WORD_CHARS, split_maqqef_compounds
from trace.model import Lexica, Token
from trace.tokenize.base import RawToken
...
Run: `flake8 src/trace/lang/hebrew tests/lang/test_hebrew_tokenize.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/lang/hebrew tests/lang/test_hebrew_tokenize.py
...
**Files:**
- Create: `src/trace/lang/hebrew/normalize.py`
- Modify: `src/trace/lang/hebrew/pack.py`
...
from trace.lang.hebrew.normalize import strip_niqqud, skeleton
from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.model import Lexica
from trace.tokenize.base import RawToken
...
```python
# src/trace/lang/hebrew/normalize.py
...
Replace the `normalize` method in `src/trace/lang/hebrew/pack.py`:
...
from trace.lang.base import LanguagePack, ScoringTier
from trace.lang.hebrew.normalize import has_gershayim, skeleton, strip_niqqud
from trace.lang.hebrew.tokenize import HEB_MID_WORD_CHARS, split_maqqef_compounds
from trace.model import Lexica, Token
from trace.tokenize.base import RawToken
...
Run: `flake8 src/trace/lang/hebrew tests/lang`
Expected: no output.

- [ ] **Step 7: Commit**

```bash
git add src/trace/lang/hebrew tests/lang/test_hebrew_normalize.py
...
**Files:**
- Create: `src/trace/lang/hebrew/data/abbreviations.json`
- Create: `src/trace/lang/hebrew/data/plene_defective.json`
- Create: `src/trace/lang/hebrew/data/__init__.py` (empty, marks dir as package)
- Modify: `src/trace/model.py` (add `Lexica.load`)
- Modify: `src/trace/lang/hebrew/pack.py` (load default lexica)
...
from trace.lang.hebrew.pack import HebrewLanguagePack, default_hebrew_lexica
from trace.model import Lexica
...
Create `src/trace/lang/hebrew/data/__init__.py` (empty file).
...
Append to `src/trace/model.py` (inside the `Lexica` class):
...
Replace the `__init__` and add the helper in `src/trace/lang/hebrew/pack.py`:
...
def default_hebrew_lexica() -> Lexica:
    data_pkg = resources.files("trace.lang.hebrew.data")
    return Lexica.load(
        {
...
Run: `flake8 src/trace tests/`
Expected: no output.

- [ ] **Step 8: Commit**

```bash
git add src/trace/lang/hebrew src/trace/model.py pyproject.toml tests/lang/test_hebrew_lexica.py
...
**Files:**
- Create: `src/trace/lang/hebrew/scoring.py`
- Modify: `src/trace/lang/hebrew/pack.py`
- Modify: `src/trace/lang/hebrew/__init__.py` (auto-register)
...
from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.lang.hebrew.scoring import (
...
from trace.lang.hebrew.normalize import skeleton
from trace.model import Reason, Token
...
def test_hebrew_pack_auto_registers_on_import():
    _reset_registry_for_tests()
    import importlib
    import trace.lang.hebrew

    importlib.reload(trace.lang.hebrew)
...
```python
# src/trace/lang/hebrew/scoring.py
...
from trace.lang.base import LanguagePack, ScoringTier, TierResult
from trace.lang.hebrew.normalize import YOD, WAW
from trace.model import Reason, Token
...
Modify `HebrewLanguagePack.scoring_tiers` in `src/trace/lang/hebrew/pack.py`:

```python
    def scoring_tiers(self) -> list[ScoringTier]:
        from trace.lang.hebrew.scoring import hebrew_scoring_tiers
        return hebrew_scoring_tiers()
```

- [ ] **Step 6: Auto-register the pack on import**

Replace `src/trace/lang/hebrew/__init__.py`:

```python
"""Hebrew language pack — auto-registers on import."""

from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.lang.registry import register_language
...
```bash
git add src/trace/lang/hebrew tests/lang/test_hebrew_scoring.py tests/lang/test_registry.py
...
**Files:**
- Create: `src/trace/score/__init__.py`
- Create: `src/trace/score/tiered.py`
...
from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.model import Reason, Token
from trace.score.tiered import tiered_score
...
```python
# src/trace/score/__init__.py
"""Scoring subpackage."""
```

```python
# src/trace/score/tiered.py
...
Run: `flake8 src/trace/score tests/test_score_tiered.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/score tests/test_score_tiered.py
...
**Files:**
- Create: `src/trace/align/__init__.py`
- Create: `src/trace/align/needleman_wunsch.py`
...
from trace.align.needleman_wunsch import AlignerConfig, align_sequences
from trace.lang.hebrew.normalize import skeleton
from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.model import Reason, Token
...
```python
# src/trace/align/needleman_wunsch.py
...
Run: `flake8 src/trace/align tests/test_aligner_dp.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/align tests/test_aligner_dp.py
...
**Files:**
- Modify: `src/trace/align/needleman_wunsch.py`
...
Run: `flake8 src/trace/align tests/test_aligner_semiglobal.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/align/needleman_wunsch.py tests/test_aligner_semiglobal.py
...
Modify `align_sequences` in `src/trace/align/needleman_wunsch.py` to consider an abbreviation-span transition into `M[i, j]`. Add a fourth traceback code (`4`) and a parallel matrix `A` (or fold into M with a marker).
...
Run: `flake8 src/trace/align tests/test_aligner_abbrev.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/align/needleman_wunsch.py tests/test_aligner_abbrev.py
...
from trace.align.needleman_wunsch import AlignerConfig, align as run_align
from trace.lang.hebrew.normalize import skeleton
from trace.lang.hebrew.pack import HebrewLanguagePack
from trace.model import AlignmentResult, Reason, Token
...
Append to `src/trace/align/needleman_wunsch.py`:

```python
import trace as _trace_pkg
from trace.model import AlignmentResult, Reason
...
Update `src/trace/align/__init__.py` to expose the wrapper:

```python
from trace.align.needleman_wunsch import AlignerConfig, align, align_sequences
...
Run: `flake8 src/trace/align tests/test_aligner_summary.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/align tests/test_aligner_summary.py
git commit -m "feat(align): add align() wrapper with summary and total_score"
```

---

## Task 16: Public API in `src/trace/__init__.py`
...
"""Tests for the public API in trace.__init__."""
...
def test_tokenize_plaintext_hebrew():
    tokens = trace.tokenize("שלום עולם", lang="hbo")
    assert len(tokens) == 2
...
def test_list_languages_includes_hebrew():
    assert "hbo" in trace.list_languages()
```

- [ ] **Step 2: Run tests; expect failure**

Run: `pytest tests/test_public_api.py -v`
Expected: FAIL — `trace.tokenize`, `trace.align`, `trace.list_languages` not exported.

- [ ] **Step 3: Wire `src/trace/__init__.py`**
...
from trace.align import AlignerConfig
from trace.align import align as _align
from trace.lang.base import LanguagePack
from trace.lang.registry import (
    UnknownLanguageError,
    get_language,
...
import trace.lang.hebrew  # noqa: F401  -- side effect: registers HBO pack
...
```bash
git add src/trace/__init__.py tests/test_public_api.py
...
**Files:**
- Create: `src/trace/io/__init__.py`
- Create: `src/trace/io/result.py`
...
import trace
from trace.io import result as result_io


def _build_result() -> "trace.AlignmentResult":
    a = trace.tokenize("שלום עולם", lang="hbo", seq_label="A")
    b = trace.tokenize("שלום עולם", lang="hbo", seq_label="B")
    return trace.align(a, b, lang="hbo", seq_a_meta={"witness_id": "W1"})
...
```python
# src/trace/io/__init__.py
"""I/O subpackage."""
```

```python
# src/trace/io/result.py
...
from trace.model import AlignmentResult
...
Run: `flake8 src/trace/io tests/io/test_result_json.py`
Expected: no output.

- [ ] **Step 6: Commit**

```bash
git add src/trace/io tests/io/__init__.py tests/io/test_result_json.py
...
**Files:**
- Create: `src/trace/io/escriptorium.py`
...
from trace.io.escriptorium import load
...
import trace
from trace.lang.base import LanguagePack
from trace.lang.registry import get_language
from trace.model import Token
...
def load(
    ...
    for region in payload.get("regions", []):
        ...
        for line in region.get("lines", []):
            ...
            line_tokens = trace.tokenize(content, lang=pack, seq_label=seq_label)
            extra: dict[str, Any] = {
...
Run: `flake8 src/trace/io tests/io/test_escriptorium.py`
Expected: no output.

- [ ] **Step 7: Commit**

```bash
git add src/trace/io/escriptorium.py tests/io/fixtures/escriptorium_minimal.json tests/io/test_escriptorium.py
...
def load(
    ...
    if w_elements:
        ...
        for position, w in enumerate(w_elements):
            ...
            if not content:
                continue
            sub = trace.tokenize(content, lang=pack, seq_label="tei")
    ...
    return trace.tokenize(body_text, lang=pack, seq_label="tei")
```
...
Run: `flake8 src/trace/io tests/io/test_tei.py`
Expected: no output.

- [ ] **Step 7: Commit**

```bash
git add src/trace/io/tei.py tests/io/fixtures/tei_with_w.xml tests/io/fixtures/tei_flow.xml tests/io/test_tei.py
...
The expected golden output is computed at first run and frozen. The test reads both witnesses, runs `trace.align`, and asserts on the summary plus the presence of the specific reasons.
...
import trace
from trace.model import Reason
...
def test_synthetic_alignment_hits_all_expected_reasons():
    ...
    a = trace.tokenize(w1, lang="hbo", seq_label="W1")
    b = trace.tokenize(w2, lang="hbo", seq_label="W2")
    result = trace.align(a, b, lang="hbo")
